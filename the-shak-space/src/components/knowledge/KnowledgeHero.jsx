  ...
  return (
    <div className="border-b border-white/[0.08] pb-5">
      ...
      <motion.div
        ...
        className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 mt-4 auto-rows-fr"
      >
        {STAT_CONFIG.map((stat, i) => {
          ...
          return (
            <motion.div
              ...
              className="min-w-0 min-h-[88px] h-full p-3.5 rounded-xl bg-white/[0.03] border border-white/[0.08] backdrop-blur-sm hover:border-white/[0.14] hover:shadow-md hover:shadow-black/20 transition-shadow flex flex-col justify-between gap-2"
            >
              <div className="flex items-center justify-between gap-2">
                <div className={`w-7 h-7 shrink-0 rounded-lg ${stat.bg} flex items-center justify-center`}>
                  <Icon size={14} className={stat.color} />
                </div>
                <span className="text-lg sm:text-xl font-black text-white font-mono leading-none tabular-nums">
                  {value}
                </span>
              </div>
              <span className="text-[8px] sm:text-[9px] font-bold text-[#A0A6B1] uppercase tracking-wide leading-snug line-clamp-2 break-words">
                {stat.label}
              </span>
            </motion.div>
          );
